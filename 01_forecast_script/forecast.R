...
elec_df_daily %>%
  group_by(type) %>% 
  plot_time_series(date_time, series, .smooth = F)
...
# * Submodels 2 Calibration ----

calibration_tbl <- submodels_2_tbl %>% 
  modeltime_calibrate(test_split, id = "type") 
...
# * Inversion on submodels_2_tbl + visualize ------------------------------

inverted_forecast_submodels_2_tbl <- submodels_2_tbl %>% 
  modeltime_forecast(
    new_data = test_split,
    actual_data = data_prepared_tbl,
    keep_data = TRUE
...
submodels_2_inversion_test_summary_metrics_tbl <- inverted_forecast_submodels_2_tbl %>% 
  filter(.key == "prediction") %>% 
  select(type, .value, series, .model_desc) %>% 
  group_by(type, .model_desc) %>% 
...
resamples_tscv %>% 
  tk_time_series_cv_plan() %>% # saved this for shiny
  plot_time_series_cv_plan(date_time, series, .title = "Time Series CV Plan - Log Transformation Applied")
...
model_tbl_tuned_resample_accuracy <- model_tbl_tuned_resamples %>% 
...
calibrated_model_ensemble_test_tbl <- model_ensemble_tbl %>% 
  modeltime_calibrate(test_split, id = "type")

calibrated_model_ensemble_test_tbl %>% 
  modeltime_accuracy()

calibrated_model_ensemble_train_tbl <- model_ensemble_tbl %>% 
  modeltime_calibrate(train_split, id = "type")


# * Comparing resampled accuracy to ensembled accuracy --------------------

model_tbl_tuned_resample_accuracy
...
forecast_ensemble_test_inverted_tbl <- model_ensemble_tbl %>% 
  modeltime_forecast(
    new_data = test_split,
    actual_data = data_prepared_tbl,
    keep_data = TRUE
...
forecast_ensemble_test_inverted_tbl %>% 
  group_by(type) %>% 
  filter(.key == "prediction") %>% 
  group_by(type) %>% 
  summarize_accuracy_metrics(truth = series, estimate = .value,
...
forecast_ensemble_inversion_test_summary_metrics_tbl <- forecast_ensemble_test_inverted_tbl %>% 
  filter(.key == "prediction") %>% 
  select(type, .value, series) %>% 
  group_by(type) %>% 
  summarize_accuracy_metrics(truth = series, estimate = .value,
                             metric_set = default_forecast_accuracy_metric_set()) %>% 
  mutate(.model_desc = "ENSEMBLE") %>% 
  select(type, .model_desc, everything())
...
# * Ensemble Training Set Residuals ---------------------------------------

calibrated_model_ensemble_train_tbl %>% 
  modeltime_residuals() %>% 
  group_by(type) %>% 
  plot_modeltime_residuals()
...
# * Ensemble Testing Set Residuals ----------------------------------------
...
# Combining submodels 2 and ensemble residuals ------------------------------

train_residuals <- calibration_tbl %>% 
  modeltime_calibrate(new_data = train_split, id = "type") %>% 
  modeltime_residuals() %>% 
  bind_rows(calibrated_model_ensemble_train_tbl %>% 
              modeltime_residuals()) 

test_residuals <- calibration_tbl %>% 
  modeltime_calibrate(new_data = test_split, id = "type") %>% 
  modeltime_residuals() %>% 
  bind_rows(calibrated_model_ensemble_test_tbl %>% 
              modeltime_residuals())
